    ...
    for i, d in enumerate(devices):
        ...
        if d['max_input_channels'] > 0:
            if name not in seen_input_names:
                ...
                is_cable_output = 'cable output' in name_lower and 'hi-fi' not in name_lower
                is_hifi_output = 'hi-fi' in name_lower and 'output' in name_lower
                inputs.append({'id': i, 'name': name, 'is_cable_output': is_cable_output, 'is_hifi_output': is_hifi_output})
        ...
        if d['max_output_channels'] > 0:
            if name not in seen_output_names:
                ...
                name_lower = name.lower()
                # CABLE Input = for Mic (Discord/Zoom)
                is_cable_input = 'cable input' in name_lower and 'hi-fi' not in name_lower
                # Hi-Fi Cable Input = for Desktop (OBS/TikTok)
                is_hifi_input = 'hi-fi' in name_lower and 'input' in name_lower
                is_vb = is_cable_input or is_hifi_input or 'vb-audio' in name_lower
                outputs.append({'id': i, 'name': name, 'is_vb': is_vb, 'is_cable_input': is_cable_input, 'is_hifi_input': is_hifi_input})
